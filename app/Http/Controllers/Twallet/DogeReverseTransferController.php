...
class DogeReverseTransferController extends Controller
{
    /**
     * @var BlockIo
     */
    ...
    {
        $balance = auth()->user()->twallet->main_doge;
        return view('transfer.t2m.doge.send',compact('balance'));
    }
    ...
    public function check(Request $request)
    {
        ...
        if($amount >= auth()->user()->twallet->main_doge){
        ...
        if($total > auth()->user()->twallet->main_doge){
        ...
        if($response['status']=='fail'){
            ...
            return view('transfer.t2m.doge.confirm',compact('amount','fee','total'));
        }
    ...
    protected function sendNow($from_address,$amount,$currency)
    {
        ...
        if($response->status == 'success'){
            ...
            $tradeBalance->update([
               'main_doge' => $tradeBalance->main_doge - $response->data->amount_withdrawn,
            ]);
        }else{
            $transaction = null;
        }
