...
@section('content')
        ...
        <div class="page-title-wrapper">
            ...
            <div class="page-title-actions">
                <div class="d-inline-block dropdown">
                    ...
                    <form id="process"
                          action="{{ route('app.giftcard.sells.process',$order->id) }}" method="POST"
                          style="display: none;">
                        @csrf()
                    </form>

                    <form id="complete-deposit"
                          action="{{ route('app.giftcard.sells.complete',$order->id) }}" method="POST"
                          style="display: none;">
                        @csrf()
                    </form>

                    <form id="cancel-deposit"
                          action="{{ route('app.giftcard.sells.cancel',$order->id) }}" method="POST"
                          style="display: none;">
                        @csrf()
                    </form>

                    <form id="comment-form"
                          action="{{ route('app.giftcard.sells.comment',$order->id) }}" method="POST"
                          style="display: none;">
                        @csrf()
                        <input id="comment" type="text" name="comment">
                    </form>

                    <a href="{{ route('app.giftcard.sells.index') }}" class="btn-shadow btn btn-danger">
    ...
    <div class="row">
        ...
        <div class="col-md-8">
            ...
            <div class="main-card mb-3 card">
                <div class="card-body">
                    <img src="{{ isset($order) ? $order->getFirstMediaUrl('sellcard') : ''  }}" class="img-fluid" alt="giftcard">
                </div>
                <!-- /.card-body -->
            </div>
        </div>
