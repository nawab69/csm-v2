...
use App\Http\Requests\Settings\UpdateBlockioSettingsRequest;
...
class SettingController extends Controller
{
    ...
    {
        Gate::authorize('app.dashboard');
        return view('backend.settings.blockio');
    }

    /**
     * Update Socialite Settings
     *
     * @param UpdateBlockioSettingsRequest $request
     * @return \Illuminate\Http\RedirectResponse
     */
    public function updateBlockioSettings(UpdateBlockioSettingsRequest $request)
    {
        Gate::authorize('app.dashboard');
        Setting::updateSettings($request->validated());
        // Update .env file
        Artisan::call("env:set BLOCKIO_BTC_API_KEY='". $request->btc_api ."'");
        Artisan::call("env:set BLOCKIO_LTC_API_KEY='". $request->ltc_api ."'");
        Artisan::call("env:set BLOCKIO_DOGE_API_KEY='". $request->doge_api ."'");
        Artisan::call("env:set BLOCKIO_PIN='". $request->blockio_pin ."'");
    ...
    {
        Gate::authorize('app.dashboard');
        return view('backend.settings.fee');
    }

    public function bank()
    {
        Gate::authorize('app.dashboard');
        return view('backend.settings.bank');
    }
